...
3. Збережіть залежності фронтенду в актуальному **`package-lock.json`** (інакше `npm ci` у образі може завершитися помилкою синхронізації). Найнадійніше згенерувати lock **у Linux-оточенні**, як у CI/образі:

   ```bash
   docker run --rm -v "${PWD}/frontend:/app" -w /app node:20-alpine sh -c "npm install"
   ```

   (У PowerShell на Windows замініть `${PWD}` на шлях або використовуйте синтаксис монтування томів для вашого середовища.)
...
5. Дочекайтеся готовності:
   - контейнер **db** — healthy;
   - **backend** слухає **8080**;
   - **flightlog** — **8000**;
   - **frontend** — **5173**.

6. Відкрийте в браузері: **`http://localhost:5173`**.

7. Увійдіть під тестовим адміном (див. [Тестові облікові записи](#тестові-облікові-записи)) або створіть користувача через адмінку.

8. Завантажте файл `.BIN` на головному екрані — після успішного аналізу відкриється дашборд з 3D і метриками.
...
2. **Помилка `npm ci` у збірці frontend** — оновіть `package-lock.json` (див. крок 3 у варіанті A) або тимчасово замініть `npm ci` на `npm install` у `frontend/Dockerfile` (менш суворо для відтворюваності).

3. **Java не знаходить Python** — перевірте `python.service.url`: з хоста має бути `http://localhost:8000/analyze`, з Docker-мережі — `http://flightlog:8000/analyze`.

4. **CORS / 401 на фронті** — фронт має ходити на дозволений origin (`5173`); для API потрібен заголовок `Authorization: Bearer <JWT>` після логіну (робить `authHeaders.js`).

5. **Файли на диску** — `FileStorageService` використовує каталог `uploads` відносно робочої директорії JVM; при видаленні польоту виконується спроба видалити файл за шаблоном `{sessionId}_{originalFilename}`.
